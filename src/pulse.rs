...
/// Pulse value in a pattern in range \[0 - 1\] where 0 means don't trigger anything, and 1 means
/// definitely do trigger something. Values within 0 - 1 maybe trigger, depending on the
/// pattern/rhythm impl.
pub type PulseValue = f32;
/// Pulse step time fraction in a pattern in range \[0 - 1\]. 1 means advance by a full step, 0.5
/// means advance by a half step, etc.
...
impl Pulse {
    ...
    pub fn len(&self) -> usize {
        match self {
            Pulse::Pulse(_) => 1,
            Pulse::SubDivision(sub_div) => sub_div.len(),
        }
    }
    ...
    fn expand_into(&self, result: &mut Vec<PulseIterItem>, step_time: PulseStepTime) {
        match self {
...
impl From<bool> for Pulse {
    fn from(value: bool) -> Self {
        Pulse::Pulse(value as u8 as PulseValue)
    }
}
