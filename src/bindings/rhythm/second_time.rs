...
use super::super::{
    unwrap::{
        ...
        pattern_from_value, pattern_repeat_count_from_value, trigger_mode_from_value,
    },
    LuaTimeoutHook,
};
...
impl SecondTimeRhythm {
    ...
    pub(crate) fn from_table(
        ...
        if table.contains_key("mode")? {
            let value = table.get::<LuaValue>("mode")?;
            let trigger_mode = trigger_mode_from_value(&value)?;
        ...
        if table.contains_key("emit")? {
            let value: LuaValue = table.get::<LuaValue>("emit")?;
